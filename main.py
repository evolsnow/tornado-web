...
class RegHandler(BaseHandler):
    ...
    '''
    def get(self):
        self.render("login_reg.html",title="用户注册", next=None, 
            head="请注册", button_name="注册", action="register")

    @tornado.gen.coroutine
    def post(self):
        db_user = self.application.db.user
        if  (yield db_user.find_one({"name": self.get_argument("name")})):
            self.write("<script>alert('用户名已注册!')</script>")
        else:
...
class LoginHandler(BaseHandler):
    ...
    '''
    def get(self):
        self.render("login_reg.html",title="用户登录", head="请登录", 
    ...
    def post(self):
        ...
        try:
            if tornado.escape.utf8(self.get_argument("password")) \
                ...
                self.write("<script>alert('密码错误!')</script>")
        except:
            self.write("<script>alert('用户不存在!')</script>")
